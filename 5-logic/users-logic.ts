...
export async function login(username, password): Promise<UserModel>{
    const query = "SELECT * FROM businesscard.users WHERE username = ? AND password = ?";
...
//    password = crypto.createHash("sha256").update(password).digest("hex");
    ...
    const query = `INSERT INTO businesscard.users(firstName, lastName, email, phone, username, password) VALUES(?,?,?,?,?,?)`;
